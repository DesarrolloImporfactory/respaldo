...
sudo tee /etc/bind/zones/$DOMINIO.merkapro.ec > /dev/null <<EOF
;
...
sudo tee /etc/apache2/sites-available/$DOMINIO.merkapro.ec.conf > /dev/null <<EOF
<VirtualHost *:80>
...
sudo a2ensite $DOMINIO.merkapro.ec.conf
sudo systemctl reload apache2

# Copiar la carpeta de la tienda al nuevo directorio del dominio
sudo cp -r /var/www/html/choclostore /var/www/html/$DOMINIO

# Cambiar permisos del directorio del dominio
sudo chmod -R 777 /var/www/html/$DOMINIO
